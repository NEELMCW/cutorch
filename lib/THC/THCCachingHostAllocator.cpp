...
struct HostAllocator
{
  ...
  std::deque<std::pair<hipEvent_t, void*>> hip_events;
  ...
  hipError_t malloc(void** ptr, size_t size)
  {
    ...
    // process outstanding hip events which may have occurred
    ...
    // note that hipHostAlloc may not touch pointer if size is 0
    *ptr = 0;
  ...
  hipError_t free(void* ptr)
  {
    ...
    if (block.event_count == 0) {
      // the block can be re-used if there are no outstanding hip events
  ...
  void emptyCache()
  {
    ...
    for (auto it = hip_events.begin(); it != hip_events.end(); ++it) {
    ...
    // all hip_events have been processed
    hip_events.clear();
  ...
  hipError_t insertEvents(Block& block)
  {
    ...
    for (auto it = streams.begin(); it != streams.end(); ++it) {
      ...
      block.event_count++;
      hip_events.emplace_back(event, block.ptr);
    }
